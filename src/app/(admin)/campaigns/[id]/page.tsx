...
import { SequenceFlowTimeline } from "@/components/portal/sequence-flow-timeline";
import type { TimelineStep } from "@/components/portal/sequence-flow-timeline";
import {
  Building2,
  CheckCircle2,
  Users,
...
export default async function CampaignDetailPage({
  ...
  // Fetch workspace name + lead sample in parallel
  const [workspace, leadData] = await Promise.all([
    prisma.workspace.findUnique({
      where: { slug: campaign.workspaceSlug },
      select: { name: true },
    }),
    campaign.targetListId
      ? getCampaignLeadSample(campaign.targetListId, campaign.workspaceSlug, 500)
  ...
  const linkedinTimelineSteps: TimelineStep[] = Array.isArray(campaign.linkedinSequence)
    ? (campaign.linkedinSequence as Array<{ position: number; type: string; body?: string; delayDays: number }>).map(step => ({
        type: "linkedin" as const,
        position: step.position,
        actionType: step.type as "connect_request" | "message" | "follow_up",
        body: step.body,
        delayDays: step.delayDays,
      }))
    : [];

  const allTimelineSteps = [...emailTimelineSteps, ...linkedinTimelineSteps];
      ...
      <div className="flex-1 min-h-0 p-6 overflow-auto">
        ...
        <div className="mt-6">
          <CampaignTabs
            ...
            leadsContent={
              <AdminLeadsTable leads={leadData.leads} totalCount={leadData.totalCount} />
            }
            sequenceContent={
              <div className="space-y-8">
                {allTimelineSteps.length > 0 ? (
                  <SequenceFlowTimeline steps={allTimelineSteps} />
                ) : (
