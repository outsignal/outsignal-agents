...
export async function POST(
  ...
  try {
    ...
    const completion = await markComplete(
      id,
      body.result ? JSON.stringify(body.result) : undefined,
    );
    if (completion.transitionedFromRunning) {
      ...
      return NextResponse.json({ ok: true, staleTransition: true });
    }
