...
2. **First: look up company by domain** to get the DB id for dedup checks:
   ```typescript
   const company = await prisma.company.findUnique({ where: { domain } });
   const companyDbId = company?.id; // may be null if company doesn't exist yet
   ```

3. For each provider (same pattern as enrichEmail but for company data):
   a. Circuit breaker check
   b. Dedup gate: If `companyDbId` exists, call `shouldEnrich(companyDbId, "company", name)` — skip if false. If `companyDbId` is null (company not in DB yet), skip the dedup check (no prior enrichment possible).
   c. Daily cap check
   d. Retry loop (same 429 backoff)
   e. On error: record (use `companyDbId ?? domain` as entityId), increment breaker, continue
   f. On success (any company field is non-null):
      - If company doesn't exist yet, create a minimal record: `prisma.company.create({ data: { domain, name: result.name ?? domain } })` and capture the new `id` as `companyDbId`
      - Then `mergeCompanyData(domain, { name, industry, headcount, description, website, location, yearFounded })`
      - `incrementDailySpend(name, result.costUsd)`
      - `recordEnrichment({ entityId: companyDbId, entityType: "company", provider: name, status: "success", ... })`
...
Note: The processNextChunk callback needs the `workspaceSlug` from the job. Extend the job object in the callback type:
