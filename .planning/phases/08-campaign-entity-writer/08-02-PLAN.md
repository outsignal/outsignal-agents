...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/knowledge/embeddings.ts"
      ...
      contains: "embedText"
    - path: "src/lib/agents/shared-tools.ts"
      provides: "Shared searchKnowledgeBase tool importable by all agents"
...
Add a `chunksRel KnowledgeChunk[]` relation to KnowledgeDocument (name it `chunksRel` to avoid conflict with existing `chunks` JSON field).
...
<task type="auto">
  ...
  <files>src/lib/knowledge/store.ts, scripts/reembed-knowledge.ts</files>
  <action>
...
- Create KnowledgeChunk records with the embeddings using raw SQL (Prisma doesn't support vector type in create):
  ...
  `
  ```
- Use `crypto.randomUUID()` for chunk IDs
...
<task type="auto">
  ...
  <files>src/lib/agents/shared-tools.ts, src/lib/agents/writer.ts, src/lib/agents/orchestrator.ts, src/lib/agents/leads.ts</files>
  <action>
