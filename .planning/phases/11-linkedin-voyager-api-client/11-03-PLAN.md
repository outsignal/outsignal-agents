...
must_haves:
  truths:
    ...
    - "Worker detects auth_expired and checkpoint_detected errors and updates sender health status"
  ...
  5. **Update `executeAction()` to use VoyagerClient:**
     ...
     private async executeAction(client: VoyagerClient, action: ActionItem): Promise<void> {
       ...
       if (result.success) {
         ...
         // Handle auth/blocking errors — invalidate cached client
         if (result.error === 'auth_expired' || result.error === 'unauthorized') {
           console.warn(`[Worker] Auth expired for sender — removing cached client`);
           this.activeClients.delete(action.id); // Will re-create with fresh cookies next tick
         }
         if (result.error === 'checkpoint_detected' || result.error === 'ip_blocked') {
           console.error(`[Worker] Sender blocked/challenged — stopping all actions`);
           this.activeClients.delete(action.id);
           // Note: The API-side health status update happens via markFailed which the API handles
         }
       }
     }
     ```
...
- Auth expired and checkpoint errors invalidate cached VoyagerClient
