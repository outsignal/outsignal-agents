...
<task type="auto">
  ...
  **Class structure:**
  ...
  3. **ConnectionStatus** — Type alias: `"connected" | "pending" | "not_connected" | "not_connectable" | "unknown"`.
     **IMPORTANT:** This uses the worker-local definition from `worker/src/linkedin-browser.ts`, NOT the shared server-side `ConnectionStatus` from `src/lib/linkedin/types.ts` (which uses `"none" | "pending" | "connected" | "failed" | "expired"`). The types are intentionally different because VoyagerClient runs only in the worker and must match the worker's existing `ActionResult`/`ConnectionStatus` contract. Add a comment in the source:
     ```typescript
