...
| Prospeo Enrichment | `POST /enrich-person`; `POST /bulk-enrich-person`; source-first bulk by `person_id` | Same Prospeo person/company objects as above, plus email status/revealed/email/verification method/MX, optional mobile, `free_enrichment`, error metadata. Live raw samples include person job history, mobile object, skills, location, and full company firmographics/technographics. | Single enrich extracts only `email`. Bulk/source-first extracts only `email`. Logs `fieldsWritten` as `["email"]` when found. | 78† | Yes. `person_id` is accepted for enrich/bulk enrich and code uses it for source-first. | Yes in enrichment logs: 4,754 total / 4,601 raw. Some skipped/failed paths lack raw. |
...
| Enrichment verification | `bounceban-verify` | 4,223 | 4,223 | 100% |
| Enrichment verification | `kitt-find` | 1,921 | 1,921 | 100% |
| Enrichment verification | `leadmagic` | 686 | 534 | 77.8% |
| Discovery | `prospeo` | 3,726 | 0 | 0% |
| Discovery | `aiark` | 3,340 | 0 | 0% |
| Discovery | `apify-leads-finder` | 2,672 | 0 | 0% |
| Discovery | `blanktag-pipeline` | 379 | 0 | 0% |
...
- `discovery-aiark`: 365 / 365 raw — these are backfill metadata payloads (`{_backfilledAt, _backfillReason, _originalSource, _personId}`), NOT original API payloads. Cannot be used as a backfill source for field extraction.
...
- `providerIds` JSON: `{ prospeoPersonId, aiarkPersonId, findymailContactId, linkedinMemberId }`.
- `headline` string.
- `seniority` string.
...
- `blanktag-pipeline` discovery: 379 live rows, zero raw payloads. This was not part of the 11-provider W3 scope, but it is likely another W2 bleed-stop target.
...
## W2 investigation required

Discovery rawResponse drop happens AFTER the adapter (which returns raw) and BEFORE the DB write (which has rawResponse: NULL). The drop is in the staging or promotion code path. W2 must trace the exact location and fix at source — fixing in adapters is wrong if the staging code already drops it.
