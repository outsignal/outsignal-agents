/**
 * Covenco Instantly HTML re-push (2026-04-15).
 *
 * Background: earlier push landed plain-text \n\n bodies into Instantly with
 * text_only=true. Admin reports Instantly UI renders them as a single block
 * (paragraph breaks not visible in editor preview). Fix: switch campaigns to
 * HTML mode (text_only=false) and convert bodies to <p>...</p> paragraph HTML.
 *
 * Scope: 9 campaigns × 3 steps × 2 variants = 54 variant bodies.
 * Variant A and Variant B share body content per existing pattern — only
 * subjects differ. Subjects stay as already pushed (not modified here).
 * Campaigns remain DRAFT (status 0). Lead queue untouched.
 *
 * Merge tags preserved: {{firstName}}, {{companyName}}.
 * Spintax preserved: {{off-site|immutable|offline}}, {{protect|stabilise|strengthen}}.
 */
...
/** Convert plain-text body (David's review doc format) to HTML paragraphs.
 *  \n\n = new <p> block. \n inside a block = <br>. */
...
async function updateOne(c: Campaign): Promise<number> {
  ...
  const htmlBodies = [toHtml(c.bodies.s1), toHtml(c.bodies.s2), toHtml(c.bodies.s3)];
  let bodyWrites = 0;
  for (let i = 0; i < 3; i++) {
    ...
    // Preserve subjects; overwrite body only
...
async function main() {
  ...
  for (const c of CAMPAIGNS) {
    try {
      ...
      console.log(`✓ ${c.name}: ${n} variant bodies updated`);
      totalWrites += n;
    } catch (e) {
  ...
  // Verify Backup Services Step 1 V A
