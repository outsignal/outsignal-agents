/**
 * Covenco Instantly — plain-text repush + enable campaign-level text-only send.
 *
 * Admin 2026-04-15: Instantly has a campaign-level UI toggle 'Send emails as
 * text only'. In the API this is the top-level `text_only` boolean (confirmed
 * via campaign GET inspection). Currently `false` on all 9 Covenco campaigns.
 *
 * This script:
 *   1. Sets `text_only: true` on all 9 campaigns (campaign-level toggle).
 *   2. Replaces every variant body (9 × 3 × 2 = 54) with plain-text David
 *      wording: `\n\n` between paragraphs, `\n` between signoff lines. Strips
 *      previously-pushed <p>/<br> HTML. No Mobile: prefix (landline).
 *
 * Subjects untouched. Campaigns stay DRAFT (status 0). Lead queue untouched.
 */
...
async function updateOne(c: Campaign): Promise<number> {
  ...
  if (current.status !== 0) {
    throw new Error(
      `REFUSING to update ${c.name} (${c.id}): status is ${current.status}, expected 0 (DRAFT)`,
    );
  }
  ...
  const bodies = [c.bodies.s1, c.bodies.s2, c.bodies.s3];
  let bodyWrites = 0;
  for (let i = 0; i < 3; i++) {
    ...
    variants[0].body = bodies[i];
    variants[1].body = bodies[i];
    bodyWrites += 2;
  }
...
async function main() {
  ...
  console.log(`\nCampaign text_only updates: ${settingUpdates}`);
  console.log(`Variant body updates: ${totalBodyWrites}`);
  console.log(`Failures: ${failures.length}`);
  for (const f of failures) console.log(`  - ${f.name}: ${f.err}`);

  // Verification pass: re-GET each campaign
  console.log("\n--- VERIFICATION ---");
  ...
  for (const c of CAMPAIGNS) {
    ...
    console.log(
      `${ok ? "OK" : "??"} ${c.name}: status=${v.status} text_only=${v.text_only}`,
    );
  }
  console.log(`\nAll text_only=true: ${allTextOnly}`);
  console.log(`All status=0 (DRAFT): ${allDraft}`);

  // Raw body echo for Backup Services Step 1 Variant A
  console.log("\n--- Backup Services Step 1 Variant A (raw stored body) ---");
  const backup = await getCampaign("578c27a2-717c-4ef2-b6d8-031b07261f4d");
  const v0 = backup.sequences[0].steps[0].variants[0];
  console.log("subject:", JSON.stringify(v0.subject));
  console.log("body:");
  console.log(v0.body);
